dnl
dnl Changed slightly from the XIPH_PATH_VORBIS macro distributed with the
dnl vorbis libraries, because the tests segfault on my machine.
dnl

dnl NORM_PATH_VORBIS([ACTION-IF-FOUND [, ACTION-IF-NOT-FOUND]])
dnl Test for libvorbis, and define VORBIS_CFLAGS and VORBIS_LIBS
dnl
AC_DEFUN([NORM_PATH_VORBIS],
[dnl 
dnl Get the cflags and libraries
dnl
AC_ARG_WITH(vorbis,
	    AC_HELP_STRING([--with-vorbis=PFX],
			   [Prefix where libvorbis is installed (optional)]),
	    vorbis_prefix="$withval", vorbis_prefix="")
AC_ARG_WITH(vorbis-libraries,
	    AC_HELP_STRING([--with-vorbis-libraries=DIR],
...
AC_ARG_WITH(vorbis-includes,
	    AC_HELP_STRING([--with-vorbis-includes=DIR],
...
AC_ARG_ENABLE(vorbistest,
	      AC_HELP_STRING([--disable-vorbistest],
			     [Do not try to compile and run a test Vorbis program]),, enable_vorbistest=yes)
...
dnl
dnl Taken from GNU grep
dnl
dnl Check to see if we use dir\file name conventtion
dnl If so, set macro HAVE_DOS_FILE_NAMES
dnl Also set the macro HAVE_DOS_FILE_CONTENTS for now,
dnl since don't know of a good way to independently check this.
dnl AC_DOSFILE()
AC_DEFUN([AC_DOSFILE],
[
